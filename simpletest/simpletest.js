...
// Only one error per failure in console
// Make failures red
...
var TinyTestHelper = {
    renderStats: function(tests, failures) {
        ...
        var summaryString = 'Ran ' + numberOfTests + ' ' + this.pluralize(numberOfTests, 'test', 's') + ': ' + successes + ' ' + this.pluralize(successes, 'success', 'es') + ', ' + failures + ' ' + this.pluralize(failures, 'failure', 's') + '.';

        var summaryElement = document.createElement('h1');
        summaryElement.textContent = summaryString;
        document.body.appendChild(summaryElement);
    },
    pluralize: function(numberOfItems, word, suffix) {
        if (numberOfItems === 1) {
            return word;
        } else {
            return word + suffix;
